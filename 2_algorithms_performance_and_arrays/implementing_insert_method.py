...
It uses another class ReservedMemory (also provided) to store the integer values in memory as bytes 
(you can convert values to byte type or you can just use integer values between 0 and 255. Both are fine. 
In the code provided the latter form is used).

When instantiating an IntArray variable, the size of the array elements can be defined. 
By default this value is 2 bytes, that gives room to store values 
from -32768 to 32767 (65536 different values in total, i.e. 16 bit range)

Some internal methods and some public methods have been already implemented. 
Task is to implement a new method "insert". The definition of the method is already present.
    ...
    def insert(self, index: int, val: int) -> None:
        ...
        if self._resmem:
            ...
            if index <= (self._size - 2):
                new_resmem.copy(self._resmem, source_index=index * self._bytes_per_element,

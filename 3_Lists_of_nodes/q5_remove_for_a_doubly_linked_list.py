"""
implement the remove method, that removes a element/node from the list at the position given as a parameter.

The remove method will accept an index position from where to remove node from the list.
The method has to return the value of the node being removed or None if the list is empty.
The method will raise a ValueError if the index is out of bounds.

The process is quite straight. Locate the nodes that before and after the node to be removed,

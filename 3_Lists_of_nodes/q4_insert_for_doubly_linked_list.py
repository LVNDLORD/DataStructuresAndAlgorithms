"""
implement the insert method, that inserts a new element/node in the list at the position given as a parameter.

The insert method will accept an index position where to insert a new node.
The value of the node is the second parameter that have to be provided to the method.
The method does not return anything, but it will raise a ValueError if the index is out of bounds.

The process is quite straight. Locate the nodes that will be before and after the new node,
and update the necessary pointers. If the header pointer is affected, it should be updated.
And the same happens with tail pointer.
"""
